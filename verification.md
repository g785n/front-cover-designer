...
The published production domain was then checked again using the same legacy parameter. It retained the nine named swatches and reported **9 URL colours connected**; a fresh local export, `report-palette-background (8).png`, was confirmed as an 8-bit RGBA PNG measuring exactly **1,066 × 735 pixels**. The final automated suite now discovers both client and server specifications: **3 test files and 10 tests passed**. The production build also completed successfully; its only notice is the existing non-blocking JavaScript bundle-size advisory.
